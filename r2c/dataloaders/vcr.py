...
class VCR(Dataset):
    def __init__(self, split, mode, only_use_relevant_dets=True, add_image_as_a_box=True, embs_to_load='bert_da',
        ...
        #############cap embedding file##########################
        self.cap_embed = os.path.join(VCR_ANNOTS_DIR, f'{self.split}_sen_embed.npy')
        ############# tokenized captions#########################
        with open(os.path.join(VCR_ANNOTS_DIR, 'caption_dict_tok.pkl'),'rb') as f:
    ...
    def __getitem__(self, index):
        ...
        cap = np.load(self.cap_embed, allow_pickle=True)
        ...
        if 'endingonly' not in self.embs_to_load:
            questions_tokenized, question_tags = zip(*[_fix_tokenization(
                # item['question'] + self.caption_dict[item['img_fn']],
                item['question'] + ['CAP'],
                # grp_items[f'ctx_{self.mode}{condition_key}{i}'],
                np.concatenate((grp_items[f'ctx_{self.mode}{condition_key}{i}'], cap[index].reshape(1, 768)), axis = 0),
                old_det_to_new_ind,
                item['objects'],
...
class VCRLoader(torch.utils.data.DataLoader):
    ...
    @classmethod
    def from_dataset(cls, data, batch_size=3, num_workers=6, num_gpus=2, **kwargs):
        loader = cls(
            dataset=data,
