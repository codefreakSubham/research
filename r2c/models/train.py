    ...
    for k in td:
        if k != 'metadata':
            td[k] = {k2: v.cuda(non_blocking=True) for k2, v in td[k].items()} if isinstance(td[k], dict) else td[k].cuda(
                non_blocking=True)
    return td
num_workers = 14 #(4 * NUM_GPUS if NUM_CPUS == 32 else 2*NUM_GPUS)-1
print(f"Using {num_workers} workers out of {NUM_CPUS} possible", flush=True)

#f = open('test.txt', 'w')
#f.write(f'\n Num gpus: {NUM_GPUS} Time:{datetime.now().strftime("%H:%M:%S")}')
#f.close()

if args.rationale:
    f = open(f'result_rat.txt', 'w')
    f.write(f'\n Started \n')
    f.close()
else:
    f = open(f'result_ans.txt', 'w')
    f.write(f'\n Started \n')
    f.close()
...
#f = open('test.txt', 'a')
#f.write(f'\n Loading data done. Time:{datetime.now().strftime("%H:%M:%S")}')
#f.write(f'\n Train loader: {type(train_loader)}')
#f.close()
...
#for epoch_num in range(1):
    ...
    model.train()
    #f = open('test.txt', 'a')   
    #f.write(f'\n Inside epoch loop. Time:{datetime.now().strftime("%H:%M:%S")}')
    #f.close()
    for b, (time_per_batch, batch) in enumerate(time_batch(train_loader if args.no_tqdm else tqdm(train_loader), reset_every=ARGS_RESET_EVERY)):
        a = time.time()
        #f = open('test.txt', 'a')
        #f.write(f'\n Batch {b} started. Time:{datetime.now().strftime("%H:%M:%S")}')
        #f.close()
        batch = _to_gpu(batch)
        optimizer.zero_grad()
    ...
    if args.rationale:
        f = open('result_rat.txt', 'a')
        f.write(f"\n Val epoch {epoch_num} has acc: {val_metric_per_epoch[-1]} and loss: {val_loss_avg}")
        f.close()
    else:
        f = open('result_ans.txt', 'a')
        f.write(f"\n Val epoch {epoch_num} has acc: {val_metric_per_epoch[-1]} and loss: {val_loss_avg}")
...
res = (et - st)/60
final_res = res / 60
print('Execution time:', final_res, 'hours')
