...
###################################################
    f = open('caption_tok.json')
    data = json.load(f)
    dict = {}

    for line in data:
        dict[(line["img_fn"])]= line["caption"]
...
###################################################
    ...
    if rationales:
        ...
        ctx = item['question'] + item['answer_choices'][answer_label] + dict[item['img_fn']]
    else:
        ctx = item['question'] + dict[item['img_fn']]
...
def data_iter(data_fn, tokenizer, max_seq_length, endingonly):
    counter = 0
    f = open('test.txt', 'a')
    f.write(f'\n Inside DataIter Func: {datetime.now().strftime("%H:%M:%S")}')
    f.close()
    with open(data_fn, 'r') as f:
        for line_no, line in enumerate(tqdm(f)):
            f = open('test.txt', 'a')
            f.write(f'\n Line no:{line_no}, Time:  {datetime.now().strftime("%H:%M:%S")}')
            f.close()
